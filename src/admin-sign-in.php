...
<head>
    ...
    <title>Walawa Cabana Lake Resort - Admin Sign In</title> 
...
<body class="min-h-screen flex items-center justify-center px-4 py-8">
    ...
    <div class="form-container animate-in bg-opacity-20 rounded-2xl shadow-xl p-8 w-full max-w-lg">
        <div class="text-center mb-8">
            ...
            <p class="text-lg tinos" style="color: var(--text-muted-color);">Admin Sign In</p>
            <div class="h-1 w-16 mx-auto mt-1" style="background-color: var(--accent-color);"></div>
            <p class="mt-3 text-sm italic tagesschrift" style="color: var(--text-muted-color);">Welcome Administrator</p>
        </div>
        ...
        <form id="signin-form" class="space-y-5">
            ...
            <div class="animate-in" style="animation-delay: 0.4s">
                <label for="username" class="block text-sm font-medium mb-1 tinos">Username</label>
                <input
                    type="text"
                    id="username"
                    name="username"
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Removed validateEmail function as it's not used for username
            ...
            if (signinForm) {
                signinForm.addEventListener('submit', function(e) {
                    e.preventDefault();
                    const usernameInput = document.getElementById('username'); // Changed from emailInput
                    const passwordInput = document.getElementById('password');
                    const username = usernameInput.value.trim(); // Changed from email
                    const password = passwordInput.value;

                    if (!username) { // Changed validation from email to username
                    ...
                    formData.append('username', username); // Changed from email to username
                    ...
                    // Ensure this endpoint can handle admin authentication or change it
                    request.open('POST', 'processes/admin-sign-in-process.php', true); 
                            ...
                            if (request.status === 200) {
                                try {
                                    ...
                                    if (response.status === 'success') {
                                        ...
                                        // Use redirect URL from response if provided, otherwise default to an admin dashboard
                                        const redirectUrl = response.redirect || 'admin-dashboard.php'; // Consider an admin-specific redirect
                                        setTimeout(() => {
            ...
            const rememberedUsername = getCookie("remember_admin_username"); // Changed cookie name
            if (rememberedUsername) {
                document.getElementById("username").value = rememberedUsername; // Target username field
